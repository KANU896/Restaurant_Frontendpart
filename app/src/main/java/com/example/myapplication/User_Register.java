...
public class User_Register extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        EditText idText = (EditText) findViewById(R.id.idText); // 아이디 입력 텍스트
        Button duplicateID = (Button) findViewById(R.id.id_confirm_btn); // 아이디 중복 체크 버튼
        ...
        EditText email = (EditText) findViewById(R.id.emailText); // 이메일 텍스트
        Button register_btn = (Button) findViewById(R.id.register_btn); // 회원가입 버튼


        // 아이디 증복체크 버튼(데이터 안에 있는 아이디 중복 검사) -> 데이터베이스 추가시 구현 예정!!
        ...
        register_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (email.getText().toString().length() == 0 ) {
                    Toast.makeText(User_Register.this, "이메일을 입력하세요", Toast.LENGTH_SHORT).show();
                    email.requestFocus();
                    return;
                }
                // 아이디 중복 체크 필수
